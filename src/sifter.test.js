import { Sifter, tokenize, prepareSearch, getScoreFunction } from "./sifter.js";
...
describe("Sifter", () => {
  test("search for a needle", () => {
    ...
    expect(foundItem.id).toStrictEqual(4);
    expect(foundItem).toHaveProperty("score");
  });

  test("parse a big long list", () => {
    const items = [
      { title: "" },
      { title: "Amount: 1" },
      { title: "Amount: 12" },
  ...
  describe("tokenize()", () => {
    ...
    test("should return an array", () => {
      ...
      expect(isArray(tokens)).toStrictEqual(true);
    });
    ...
    describe("returned tokens", () => {
      ...
      test("string property", () => {
        expect(head(tokens)).toHaveProperty("string");
        expect(typeof head(tokens).string).toStrictEqual("string");
        expect(head(tokens).string).toStrictEqual("hello");
        expect(head(tail(tokens)).string).toStrictEqual("world");
      });
  ...
  describe("getScoreFunction()", () => {
    test('should acknowledge AND "conjunction" option', () => {
    ...
    test('should acknowledge or "conjunction" option', () => {
      const score = getScoreFunction("one two", {
        fields: ["a", "b"],
        conjunction: "or"
      });
    ...
    test("with query and options should return a function that returns a number", () => {
    ...
    test("with pre-prepared search should return a function that returns a number", () => {
  ...
  describe("search()", () => {
    ...
    test("should allow word boundaries to be respected", () => {
    ...
    describe("sorting", () => {
      test("it should respect 'sort_empty' option when query absent", () => {
        const sifter = new Sifter([
          { field: "aaa" },
          { field: "add" },
          { field: "abb" }
      ...
      test("should work with one field (as object)", () => {
        const sifter = new Sifter([
          { field: "aaa" },
          { field: "add" },
          { field: "abb" }
      ...
      test("should work with one field (as array)", () => {
        const sifter = new Sifter([
          { field: "aaa" },
          { field: "add" },
          { field: "abb" }
      ...
      test("should work with multiple fields and respect priority", () => {
        const sifter = new Sifter([
          { a: "bbb", b: "bbb" },
          { a: "bbb", b: "ccc" },
          { a: "bbb", b: "aaa" },
      ...
      test("should respect numeric fields", () => {
        const sifter = new Sifter([
          { field: 1.0 },
          { field: 12.9 },
          { field: 9.1 },
      ...
      test("should respect sort direction", () => {
        const sifter = new Sifter([
          { a: "bbb", b: "rrr" },
          { a: "bbb", b: "aaa" },
          { a: "aaa", b: "rrr" },
      ...
      test('should add implicit "$score" field when query present', () => {
      ...
      test('should not add implicit "$score" field if explicitly given', () => {
        const sifter = new Sifter([
          { field: "boooo" },
          { field: "yoo" },
          { field: "aaa" }
    ...
    describe("returned results", () => {
      ...
      test("should not vary when using an array vs an object as a data source", () => {
      ...
      describe('"items" array', () => {
        ...
        test("should include entire set if no query provided", () => {
          expect(resultAll.items).toHaveLength(5);
        });
        test('should not have a length that exceeds "limit" option', () => {
          expect(result.items.length).toBeLessThanOrEqual(options.limit);
        });
        test("should not contain any items with a score not equal to 1 (without query)", () => {
          for (let i = 0, n = resultAll.items.length; i < n; i++) {
            expect(resultAll.items[i].score).toStrictEqual(1);
          }
        });
        test("should not contain any items with a score of zero (with query)", () => {
          for (let i = 0, n = result.items.length; i < n; i++) {
            expect(result.items[i].score).not.toStrictEqual(0);
          }
        });
        test("should be empty when no results match", () => {
        ...
        describe("elements", () => {
          ...
          describe('"score" property', () => {
            test("should exist and be a number", () => {
        ...
        describe("options", () => {
          test("should not be a reference to original options", () => {
            expect(result.options).not.toBe(options);
          });

          test("should match original search options", () => {
        ...
        describe('"query"', () => {
          test("should match original query", () => {
            expect(result.query).toBe("switzerland europe");
          });
        });

        describe('"total"', () => {
          test("should be an integer and valid", () => {
